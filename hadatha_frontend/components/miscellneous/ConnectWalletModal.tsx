...
const ConnectWalletModal = ({ open, setOpen, redirectUrl }: { open: boolean, setOpen: (open: boolean) => void, redirectUrl?: string }) => {
    ...
    useEffect(() => {
        if (account?.address && open && !isLoading && !isRedirecting) {
    ...
    }, [account?.address, open, hasAccount, isLoading, redirectUrl, router, setOpen, isRedirecting]);
                ...
                <div className="w-full flex flex-col gap-2 items-center">
                    {(isLoading || isRedirecting) ? (
                        <div className="flex items-center justify-center gap-2 text-white bg-white/20 rounded-full w-full py-4">
                            ...
                            {isRedirecting ? "Redirecting..." : "Checking account..."}
                        </div>
                    ) : (
                        <>
                            ...
                            {!account?.address ? (
                                <ConnectButton
                                    className="w-full lg:hidden! rounded-full! px-12! min-h-[50px] cursor-pointer bg-white/5! backdrop-blur-lg! text-white! border border-white/10 active:scale-95 hover:scale-x-105 transition-all duration-300 font-sans!"
                                    connectText={
                                ...
                                <div className="flex items-center justify-center gap-2 text-white bg-white/20 rounded-full w-full py-4 lg:hidden">
                            ...
                            {Array.isArray(regularWallets) && regularWallets.length !== 0 ? regularWallets.map((wallet) => (
                                <Button
                                    ...
                                    disabled={!!account}
                                    className="w-full hidden rounded-full px-12! py-5! min-h-[50px] cursor-pointer bg-white/5 backdrop-blur-lg border border-white/10 active:scale-95 hover:scale-x-105 transition-all duration-300 lg:flex justify-center items-center gap-2 disabled:cursor-not-allowed disabled:opacity-50"
                                >
                    ...
                    <Button disabled className="w-full rounded-full px-12! py-5! min-h-[50px] cursor-pointer bg-white/5 backdrop-blur-lg border border-white/10 transition-all duration-300 flex justify-center items-center gap-2 opacity-50 disabled:cursor-not-allowed">
                        <FcGoogle size={24} color="white" />
                        <span>Google Sign In <span className="text-xs text-white/50">(Coming Soon)</span></span>
                    </Button>
                    <Button disabled className="w-full rounded-full px-12! py-5! min-h-[50px] cursor-pointer bg-white/5 backdrop-blur-lg border border-white/10 transition-all duration-300 flex justify-center items-center gap-2 opacity-50 disabled:cursor-not-allowed">
                        <FaFacebook size={24} color="white" />
                        <span>Facebook Sign In <span className="text-xs text-white/50">(Coming Soon)</span></span>
                    </Button>
                </div>
            </div>
        </ModalWrapper>
